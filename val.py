...
weights = '/root/autodl-tmp/cspsg-master/runs/train/exp327/weights/best.pt'
val_path = '/root/autodl-tmp/cspsg-master/datasets/cityscapes/leftImg8bit/val'
test_path = '/root/autodl-tmp/cspsg-master/datasets/cityscapes/leftImg8bit/test'
#test_path = '/root/autodl-tmp/cspsg-master/datasets/coarse'
batch_size = 1
workers = 0
...
test_dataset = LoadImagesAndLabels_sg(test_path, phase=phase)
...
with torch.no_grad():
    if phase == 'test':
        ...
        for i, (imgs, labels, imgs_f, paths) in enumerate(val_loader):  # batch -------------------------------------------------------------
           ...
           # cv2.imwrite('results/2/'+file_name, preds1)
           # cv2.imwrite('results/2_color/'+file_name, preds2[:,:,::-1])
        ...
        for i, (imgs, labels, imgs_f, paths) in enumerate(test_loader):  # batch -------------------------------------------------------------
            ...
            cv2.imwrite('results/ddexp_modal_flip_test/'+file_name, preds1)
            cv2.imwrite('results/coarse_color/'+file_name, preds2[:,:,::-1])
            metrics.update(labels[0], preds)
            print(i)
    elif phase == 'speed':
        sum_time = 0
