...
#weights = '/root/autodl-tmp/cspsg-master/runs/train/exp276/weights/269.pt'
#weights = '/root/autodl-tmp/cspsg-master/runs/train/exp279/weights/347.pt'
#weights = '/root/autodl-tmp/cspsg-master/runs/train/exp283/weights/87.pt'
#weights = '/root/autodl-tmp/cspsg-master/runs/train/exp288/weights/72.pt'
weights = '/root/autodl-tmp/cspsg-master/runs/train/exp299/weights/1.pt'
val_path = '/root/autodl-tmp/cspsg-master/datasets/CamVid/camvid_val.txt'
test_path = '/root/autodl-tmp/cspsg-master/datasets/CamVid/camvid_test.txt'
batch_size = 1
workers = 0
...
test_dataset = LoadImagesAndLabels_sg_cam(test_path, phase=phase)
...
with torch.no_grad():
    if phase == 'test':
        ...
        for i, (imgs, labels, imgs_f, paths) in enumerate(test_loader):  # batch -------------------------------------------------------------
            imgs = torch.cat((imgs, imgs_f), dim=0)
           ...
           # cv2.imwrite('results/cam1/'+file_name, preds1)
            cv2.imwrite('results/cam2_color/'+file_name, preds2[:,:,::-1])
            metrics.update(labels[0], preds)
            print(i)
    elif phase == 'speed':
        sum_time = 0
