...
#from utils.cga import SpatialAttention, ChannelAttention, PixelAttention
...
class C3(nn.Module):
    ...
    def __init__(self, c1, c2, n=1, shortcut=True, g=1, e=0.5):  # ch_in, ch_out, number, shortcut, groups, expansion
        ...
        self.att=MSCAAttention(c_)

    def forward(self, x):
        return self.cv3(torch.cat((self.m(self.cv1(x)), self.att(self.cv2(x))), 1))
...
class SPPF(nn.Module):
    ...
    def __init__(self, c1, c2, k=5):  # equivalent to SPP(k=(5, 9, 13))
        ...
        self.ps = PolarizedSelfAttention(c2)
    ...
    def forward(self, x):
        ...
        with warnings.catch_warnings():
            ...
            y1 = self.m(x)
            y2 = self.m(y1)
            return self.ps(self.cv2(torch.cat((x, y1, y2, self.m(y2)), 1)))
...
class SPPCSPC_group(nn.Module):
    def __init__(self, c1, c2, n=1, shortcut=False, g=1, e=0.5, k=(5, 9, 13)):
        ...
        self.cv2 = Conv(c1, c_, 1, 1, g=2)
        self.cv3 = Conv(c_, c_, 3, 1, g=2)
        self.cv4 = Conv(c_, c_, 1, 1, g=2)
        self.m = nn.ModuleList([nn.MaxPool2d(kernel_size=x, stride=1, padding=x // 2) for x in k])
        self.cv5 = Conv(4 * c_, c_, 1, 1, g=2)
        self.cv6 = Conv(c_, c_, 3, 1, g=2)
        self.cv7 = Conv(2 * c_, c2, 1, 1, g=2)
...
class CBAMC3(nn.Module):
    ...
    def __init__(self, c1, c2, n=1, shortcut=True, g=1, e=0.5):  # ch_in, ch_out, number, shortcut, groups, expansion
        ...
        self.rfa = RFAConv(c1, c_, 1, 1)

        # self.m = nn.Sequential(*[CrossConv(c_, c_, 3, 1, g, 1.0, shortcut) for _ in range(n)])

    def forward(self, x):
   		# 将最后的标准卷积模块改为了注意力机制提取特征
        return self.cv3(torch.cat((self.m(self.cv1(x)), self.rfa(x)), dim=1))
